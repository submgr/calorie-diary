...
<ion-page>
    ...
    <ion-content :fullscreen="true">
        ...
        <div v-if="currentListView == 'shared'">
            <DynamicScroller style="margin-top: 3vh" :items="shared_items" :min-item-size="100" gridItems=2 class="scroller" keyField="code">
                <template v-slot="{ item, index, active }">
                    <DynamicScrollerItem :item="item" :min-item-size="500" :active="active" :gridItems="2" :data-index="index">
        ...
        <div v-if="currentListView == 'local'">
            <DynamicScroller v-if="local_items.length > 0" style="margin-top: 3vh" :items="local_items" :min-item-size="100" gridItems=2 class="scroller" keyField="code">
                <template v-slot="{ item, index, active }">
                    <DynamicScrollerItem :item="item" :min-item-size="500" :active="active" :gridItems="2" :data-index="index">
                        <ion-item @click="selectedEatenElement(item.friendly_name, item.calories, item.proteins, item.fats, item.carbohydrates)">
                            <ion-icon :icon="ellipseOutline"></ion-icon>
    ...
    <ion-modal @willDismiss="Modal_onWillDismiss" :is-open="modalState == 'confirmEatenAdd'">
